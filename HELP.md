...
등록할 때 현재 서버가 로그 저장 폴더로 복사되며, 기존에 등록된 서버와 이전 버전 파일은 새 서버로 교체됩니다.
...
서버와 확장의 프로토콜 버전이 다르면 확장이 서버에 자동 업데이트를 요청합니다.
서버는 GitHub Releases에서 현재 운영체제와 CPU에 맞는 서버 압축파일을 검증하여 설치한 뒤 자동으로 재시작합니다.
자동 업데이트를 지원하지 않는 구버전 서버는 한 번 수동으로 업데이트해야 합니다.
